...
@torch.no_grad()
def count_ops_and_params(model, example_inputs, ignore_list=[], layer_wise=False):
    global CUSTOM_MODULES_MAPPING
    ori_model = model 
    model = copy.deepcopy(model) # deepcopy to avoid changing the original model
    ignore_modules = [module for name, module in model.named_modules() if name in ignore_list]
