...
def get_flops_pytorch(model, input_res,
    ...
    try:
        if isinstance(batch, dict):
            ...
            _ = flops_model(batch)
        flops_count, params_count = flops_model.compute_average_flops_cost(ignore_list=ignore_modules)
...
def get_model_parameters_number(model, ignore_list=[]):
    if ignore_list == []:
        params_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
    else:
        params_num = 0
        for module in model.modules():
            if list(module.children()) == [] and module not in ignore_list:
...
def compute_average_flops_cost(self, ignore_list):
    """
    ...
    params_sum = get_model_parameters_number(self, ignore_list)
